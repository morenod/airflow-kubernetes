...
from openshift_nightlies.tasks.install.aro import aro
...
class AroNightlyDAG(AbstractOpenshiftNightlyDAG):
    def build(self):
        ...
        if self.config.cleanup_on_success:
            cleanup_cluster = installer.get_cleanup_task()
            install_cluster >> aro_post_installation >> benchmarks >> cleanup_cluster >> final_status
        else:
            install_cluster >> aro_post_installation >> benchmarks
...
def build_releases():
    ...
    for release in release_manifest.get_releases():
        ...
        elif openshift_release.platform == "aro":
